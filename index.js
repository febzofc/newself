...
const { getBuffer, h2k, generateMessageID, getGroupAdmins, getRandom, banner, start, info, success, close, author } = require('./lib/function')
...
const { Miminnya, BotName, fake, ownerNumber, targetpc, lolkey } = require('./setting.json')
...
//================================================================================//
        ...
        const pushname = pebz.contacts[sender] != undefined ? pebz.contacts[sender].vname || pebz.contacts[sender].notify: undefined
...
//================================================================================//
 ...
 //+++ || FAKE TROLI         
            const troli =  {key: { fromMe: false,remoteJid: "status@broadcast", participant: '0@s.whatsapp.net'}, message: {orderMessage: {itemCount: 2021, status: 200, thumbnail: gambar, surface: 200, message: `» Made By ItsMeFebz\n» ${ucapanWaktu}${pushname}`, orderTitle: 'LordPebri', sellerJid: '0@s.whatsapp.net'} } }                	
...
//+++++B A T A S  A N T I L I N K                 	        				      	        				   	        				                 	        				      	        		

//+++ MENU HORROR NO PREFIX✓		   	        				 
switch(is) {
...
case 'horor':
case 'menu':
reply(`${pushname} pake ${prefix} ajc`)
           case `${prefix}simi`:           
           case 'simi':
           case 'sim':
...
» ${p}WIT ${timeWit}${p}
» ${p}${tanggal()}${p}
${p}⏳Runtime ${kyun(run)}${p}

*_©${author}_*
`           
           but = [
          { buttonId: `${prefix}owner1`, buttonText: { displayText: 'CREATOR👑️' }, type: 1 },
           { buttonId: `${prefix}usebot`, buttonText: { displayText: 'HOW TO USE⚙️' }, type: 1 },
...
> ${prefix}upswvideo`
 but = [
         { buttonId: `${prefix}menu`, buttonText: { displayText: '◀️BACK TO MENU' }, type: 1 }
        ]
         sendButton(from, tekkk, fake, but)
         break
            case 'ruls':
txt = `*「 PERATURAN BOT 」*
    ...
    case 'tourl':
        ...
        sendButLocation(from, der, 'GK JALANIN WAJIB DONATE',todz, but, {quoted: mek})
          ...
          case 'quotesimg':
           ...
           gbutsan = [{buttonId: `${prefix}quotesimg`, buttonText: {displayText: 'NEXT⏩'}, type: 1}]
            pto =await pebz.prepareMessage(from, todzi, image)    
            gbuttonan = {
        imageMessage: pto.message.imageMessage,
        contentText: `Menampilkan ${command}`,
        footerText: 'Klil Next Untuk Gambar Selanjutnya☕',
        buttons: gbutsan,
        headerType: 4
}
...
└ *Wiki :* ${resa[0].wiki}`
  ...
  if (!isQuotedAudio) return reply('Reply Audionya')
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						...
						pebz.sendMessage(from, topt, audio, {mimetype: 'audio/mp4', quoted: mek, ptt:true})
						})
